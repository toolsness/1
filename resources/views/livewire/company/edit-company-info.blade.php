<div class="pb-16">
    <div class="container max-w-2xl mx-auto mt-8">
        <h2 class="mb-4 mb-5 text-2xl font-bold text-center">Edit Company Information</h2>
        @if(!$isEditing)
        ...
        @else
            <form wire:submit.prevent="confirmUpdate">
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <input type="text" id="companyId" value="{{ $company->id }}" class="w-full px-3 py-2 border rounded hover:bg-gray-100 disabled:opacity-75 disabled:cursor-not-allowed" disabled>
                        @error('id') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
                        <label for="companyUserNameKanji" class="block my-2">Name of {{ str_replace('Company', '', $userType) }} (Kanji)</label>
                    </div>
                    <div>
                        <input type="text" id="companyUserNameKanji" wire:model.lazy="companyUserNameKanji" class="w-full px-3 py-2 border rounded">
                        @error('companyUserNameKanji') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
                        <label for="companyUserNameKatakana" class="block my-2">Name of {{ str_replace('Company', '', $userType) }} (Katakana)</label>
                    </div>
                    <div>
                        <input type="text" id="companyUserNameKatakana" wire:model.lazy="companyUserNameKatakana" class="w-full px-3 py-2 border rounded">
                        @error('companyUserNameKatakana') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
                        <label for="companyUsercontactPhone" class="block my-2">{{ str_replace('Company', '', $userType) }} Contact Phone</label>
                    </div>
                    <div>
                        <input type="text" id="companyUsercontactPhone" wire:model.lazy="companyUserContactPhone" class="w-full px-3 py-2 border rounded">
                        @error('companyUserContactPhone') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
                    ...
                    <div>
                        <input type="text" id="address" wire:model.lazy="address" class="w-full px-3 py-2 border rounded">
                    ...
                    <div>
                        <input type="url" id="website" wire:model.lazy="website" class="w-full px-3 py-2 border rounded">
                    ...
                    <div>
                        <input type="email" id="contactEmail" wire:model.lazy="contactEmail" class="w-full px-3 py-2 border rounded">
                        @error('contactEmail') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
                        <label for="contactPhone" class="block my-2">Contact Phone</label>
                    </div>
                    <div>
                        <input type="text" id="contactPhone" wire:model.lazy="contactPhone" class="w-full px-3 py-2 border rounded">
                        @error('contactPhone') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="relative w-32 h-32">
                        @if ($tempLogo)
                            <img src="{{ $tempLogo->temporaryUrl() }}" alt="Temp Company Logo" class="object-contain w-full h-full border border-gray-300 rounded shadow-sm">
                        @else
                            <img src="{{ $logo ? Storage::url($logo) : asset('placeholder.png') }}" alt="Company Logo" class="object-contain w-full h-full border border-gray-300 rounded shadow-sm">
                        @endif

                        <label for="tempLogo" class="absolute top-0 right-0 p-1 text-white bg-black bg-opacity-50 rounded-full cursor-pointer hover:bg-opacity-75">
                            <i class="fas fa-camera"></i>
                            <input type="file" id="tempLogo" wire:model.lazy="tempLogo" accept="image/*" class="hidden">
                        </label>
                    </div>
                    @error('tempLogo') <span class="mt-2 text-red-500">{{ $message }}</span> @enderror
                </div>
    ...
    @if($showPasswordConfirmation)
        <div class="fixed inset-0 w-full h-full overflow-y-auto bg-gray-600 bg-opacity-50" id="password-modal">
            <div class="relative p-5 mx-auto bg-white border rounded-md shadow-lg top-20 w-96">
                <div class="mt-3 text-center">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">Confirm Password</h3>
                    ...
                    <div class="items-center px-4 py-3">
                        <button wire:click="updateCompanyInfo" wire:loading.remove wire:target="updateCompanyInfo" class="w-full px-4 py-2 text-base font-medium text-white bg-blue-500 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300">
                            Confirm Update
                        </button>
                        <div wire:loading wire:target="updateCompanyInfo">
                            <span class="font-bold text-red-500"><i class="fa fa-spinner fa-spin"></i> updating...</span>
                        </div>


                        <button wire:click="cancelPasswordConfirmation" class="w-full px-4 py-2 mt-2 text-base font-medium text-white bg-red-500 rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-300">
                            Cancel
                        </button>
                    </div>
                </div>
