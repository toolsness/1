<div>
    <main class="flex flex-col items-center flex-grow max-w-md py-12 mx-auto md:max-w-lg">
        <h2 class="mb-8 text-2xl font-bold text-gray-800">Student Login</h2>
        <form wire:submit.prevent="login" class="w-full max-w-md">
            ...
            <div class="mb-8 grid grid-cols-[1fr_auto] gap-5 items-center">
                ...
                <div class="relative flex flex-col" x-data="{ showPassword: false }">
                    <input
                        class="appearance-none border rounded w-[300px] py-3 px-4 pr-10 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        wire:model.defer="password" x-bind:type="showPassword ? 'text' : 'password'" placeholder="Enter your password" />
                    <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 px-3 py-2">
            ...
            <div class="items-center mb-6 text-center">
                <p class="text-gray-700">If you are not yet a member, <a class="text-blue-500 hover:underline hover:uppercase" title="New Student Registration" href="{{ route('student.new-member-registration') }}">click here.</a></p>
            </div>
            <div class="flex items-center justify-between mb-6">
                ...
                <div>
                    <a href="{{ route('student.password.request') }}" title="If you have forgotten your password please click here to reset it!" class="text-blue-500 hover:underline hover:uppercase">Forgot Password?</a>
                </div>
            </div>
